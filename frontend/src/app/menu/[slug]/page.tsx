...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchDish = async () => {
      try {
        setIsLoading(true);
        const res: any = await api.get(`/menu/items/${slug}`);
        if (res.success && res.data) {
          setDish(res.data);
          setSelectedImage(res.data.image);
          if (res.data.portions && res.data.portions.length > 0) {
            setSelectedPortion(res.data.portions[0]);
          }
        } else {
  ...
  const handleDirectWhatsApp = () => {
    handleAddToCart();
    const message = `হ্যালো ঘরোয়া রেস্টুরেন্ট! আমি *${dish.nameEn}* অর্ডার করতে চাই।\n- পরিমাপ: ${selectedPortion?.nameEn || 'Regular'}\n- পরিমাণ: ${quantity} টি\n- অতিরিক্ত যোগ: ${selectedAddOns.map((a) => a.nameEn).join(', ') || 'নেই'}\n- মোট মূল্য: ৳${grandTotal}`;
    const url = `https://wa.me/8801973255888?text=${encodeURIComponent(message)}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-12 pb-24 sm:pb-16">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div>
            <div className="flex items-center justify-between gap-4">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 font-serif">
                {language === 'bn' ? dish.nameBn : dish.nameEn}
              </h1>

              {dish.isBestseller && (
                <span className="hidden sm:inline-block text-[11px] font-bold bg-red-100 text-[#900C19] px-2.5 py-0.5 rounded-full">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-600 mt-2">
              <div className="flex items-center gap-1 text-amber-500">
                ...
                <span className="font-bold text-slate-900">{dish.rating || 4.9}</span>
              </div>
            ...
            {/* Price */}
            <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 font-mono mt-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            ...
            >
              ...
              <span>{language === 'bn' ? 'WhatsApp এ অর্ডার' : 'WhatsApp Order'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-8 border-t border-slate-200">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs text-slate-700">
              <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60">
                ...
                <span>{language === 'bn' ? 'প্রিমিয়াম কোয়ালিটি খাসির মাংস' : 'Premium Quality Mutton'}</span>
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60">
                <Sparkles className="w-4 h-4 text-[#900C19]" />
                <span>{language === 'bn' ? 'সুগন্ধি ঐতিহ্যবাহী গোপন মশলা' : 'Aromatic Traditional Spices'}</span>
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60">
                <Flame className="w-4 h-4 text-[#900C19]" />
                <span>{language === 'bn' ? 'ধীমে আঁচে সুসিদ্ধ ও নরম' : 'Perfect Cooked & Tender'}</span>
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60">
                <CheckCircle2 className="w-4 h-4 text-[#900C19]" />
                <span>{language === 'bn' ? 'স্বাস্থ্যসম্মত ও শতভাগ হালাল' : 'Hygienically Prepared 100% Halal'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-center text-xs">
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/60">
                ...
                <div className="font-bold text-slate-800 text-sm font-mono mt-1">
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/60">
                <div className="text-slate-400 text-[10px]">Fiber</div>
                <div className="font-bold text-slate-800 text-sm font-mono mt-1">
        ...
        {/* Right Side: Customer Reviews Breakdown (Reference 1) */}
        <div className="lg:col-span-5 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold text-[#900C19] hover:underline"
            >
          ...
          <div className="flex items-center gap-6 pb-6 border-b border-slate-100">
            <div className="text-center">
              <div className="text-4xl font-extrabold text-slate-900 font-serif">
                {dish.rating || 4.9}
              </div>
