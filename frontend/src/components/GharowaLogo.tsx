...
export default function GharowaLogo({ className = '', size = 44, variant = 'header' }: GharowaLogoProps) {
  if (variant === 'full') {
    return (
      <svg
        width={size * 1.2}
      ...
      >
        ...
        {/* Brick Lines Texture (Horizontal & Vertical) */}
  ...
  // Circular Emblem Style (Matching Header Reference 2)
  return (
    <div
      style={{ width: size, height: size }}
      className={`relative rounded-full bg-white border-2 border-amber-400/60 p-1 flex items-center justify-center shrink-0 shadow-md ${className}`}
    >
      <svg
        viewBox="0 0 200 200"
        fill="none"
