  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        {dish.isBestseller ? (
          <div className="absolute top-2.5 left-2.5 bg-[#900C19] text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1">
            <Flame className="w-3 h-3 text-amber-300 fill-current" />
            <span>{language === 'bn' ? 'বেস্টসেলার' : 'Best Seller'}</span>
          </div>
        ) : dish.isFeatured ? (
          <div className="absolute top-2.5 left-2.5 bg-amber-600 text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full shadow-md">
      ...
      <div className="p-3.5 sm:p-4 flex-1 flex flex-col justify-between space-y-2.5">
        <div>
          {/* Dish Name */}
          <h3 className="text-sm sm:text-base font-bold text-slate-900 font-bengali group-hover:text-[#900C19] transition-colors line-clamp-1">
          ...
          <div className="flex items-center gap-1 text-xs text-slate-500 mt-1">
            ...
            <span className="font-bold text-slate-800 text-xs">{rating}</span>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 flex items-center gap-1 shadow-sm ${
              inCartQty > 0
